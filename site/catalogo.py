...
CONTAGENS = {"municípios", "cursos", "instituições", "alunos", "vagas/ano",
             ...
             "estabelecimentos", "unidades", "equipes"}
...
INDICADORES = [
    ...
    _i("ICAB", "ICAB", "Cobertura assistencial — força de trabalho",
       "Cobertura — saúde",
       "Fração dos municípios do estado com ao menos um cirurgião-dentista "
       "vinculado ao SUS. Responde onde existe profissional em atividade capaz "
       "de absorver quem se forma. Conta pela família CBO 2232, por prefixo: "
       "casar por texto capturaria professores e pesquisadores de Odontologia, "
       "que não fazem assistência, e também auxiliares e técnicos em saúde "
       "bucal, que são outra ocupação. Leia junto com a densidade: presença e "
       "quantidade são perguntas diferentes.",
    ...
    # ------------------------------------------------------------ capacidade
    _i("vagas_total", "Vagas totais", "Capacidade total de vagas anuais",
       "Capacidade",
       "Todas as vagas anuais de Odontologia. Como não há oferta a distância "
       "no país, este número é inteiramente a capacidade presencial — o campo "
       "de EaD existe, e vale zero.",
    ...
    _i("vagas_presencial", "Vagas presenciais", "Vagas em cursos presenciais",
       ...
       "toda a formação em Odontologia no país, sem exceção.",
       "vagas/ano", "contextual", "Censo INEP", dec=0,
    ...
    _i("vagas_ead", "Vagas EaD", "Vagas a distância", "Ensino a distância",
       ...
       "mantenedora. Em Odontologia são ZERO no país inteiro, contra 118.747 "
       "presenciais. Zero aqui significa que o estado foi medido e não tem "
       "nenhuma vaga EaD — é medida, não lacuna. O campo continua publicado "
       "justamente para que a ausência seja verificável.",
       "vagas/ano", "contextual", "Censo INEP", dec=0, aliases=["vagas ead"]),

    _i("pct_ead", "% EaD", "Participação da EaD na capacidade",
       "Ensino a distância",
       "Percentual das vagas que são a distância. Em Odontologia é 0% em "
       "todas as UFs. Para comparar: em Administração passa de 67%, em "
       "Fonoaudiologia fica em 43% e em Psicologia, 0,2%. Por ser constante, "
       "este indicador fica fora da matriz de correlação e dos mapas: sem "
       "variância não há associação a medir.",
    ...
    _i("HHI_mantenedora", "HHI (mantenedora)", "Concentração por mantenedora",
       ...
       "Em Odontologia os dois valores ficam próximos, e isso é achado, não "
       "erro de conta: a distância entre eles mede o peso dos grandes grupos "
       "de EaD, e aqui não existe EaD alguma.",
    ...
    _i("municipios_oferta", "Municípios com curso",
       ...
       "Municípios onde existe ao menos um curso presencial: 303 no país, de "
       "5.571.",
    ...
    _i("municipios_deserto", "Municípios sem curso",
       ...
       "Municípios do estado sem nenhum curso presencial de Odontologia. Como "
       "não há EaD no curso, aqui não existe a alternativa do polo: sem curso "
       "presencial, não há oferta nenhuma no município.",
    ...
    # ------------------------------------------------------- cobertura: saúde
    _i("municipios_com_dentista", "Municípios com dentista",
       "Municípios com cirurgião-dentista no SUS", "Cobertura — saúde",
       "Municípios com ao menos um profissional da família CBO 2232 vinculado "
       "ao SUS no CNES.",
       "municípios", "maior", "CNES/DATASUS", dec=0),

    _i("dentistas_sus", "Dentistas no SUS",
       "Cirurgiões-dentistas vinculados ao SUS", "Cobertura — saúde",
       "Profissionais distintos, não vínculos: o mesmo dentista com três "
       "vínculos conta uma vez. Contar vínculos transformaria precariedade em "
       "abundância.",
       "profissionais", "maior", "CNES/DATASUS", dec=0),

    _i("dentistas_por_100k", "Dentistas / 100 mil hab.",
       "Densidade de cirurgiões-dentistas no SUS", "Cobertura — saúde",
       "Cirurgiões-dentistas vinculados ao SUS por 100 mil habitantes. É a "
       "medida que separa os estados: ter um dentista e ter trinta são "
       "situações diferentes, e só a densidade enxerga a diferença.",
    ...
    _i("municipios_com_esb", "Municípios com equipe de saúde bucal",
       ...
       "Municípios com ao menos uma equipe de saúde bucal cadastrada e ativa "
       "no CNES. Equipe com data de desativação não é contada: estrutura "
       "extinta não é cobertura atual.",
       "municípios", "maior", "CNES/DATASUS", dec=0,
       aliases=["esb", "equipe de saúde bucal"]),

    _i("equipes_esb_total", "Equipes de saúde bucal",
       "Equipes de saúde bucal ativas", "Cobertura — saúde",
       "Equipes distintas de saúde bucal cadastradas e ativas no CNES no "
       "estado.",
    ...
    _i("IDD", "IDD", "Indicador de Diferença entre Desempenhos", "Qualidade",
       ...
       "previa. No ciclo 2023 de Odontologia, 445 dos 452 cursos avaliados têm "
       "IDD; os demais ficam FORA das médias ponderadas do estado, em vez de "
       "entrarem como zero — lacuna não é desempenho ruim.",
       "0 a 5", "maior", "CPC INEP", dec=3, min=0, max=5, aliases=["idd"]),

    _i("n_cursos_avaliados", "Cursos avaliados", "Cursos no ciclo do CPC",
       "Qualidade",
       "Cursos do estado avaliados no ciclo do CPC. Odontologia está no ciclo "
       "de 2023, o da saúde e das engenharias — e todos os 452 cursos "
       "avaliados são presenciais.",
    ...
    _i("cursos_presencial", "Cursos presenciais",
       ...
       "Cursos presenciais de Odontologia no município.",
       "cursos", "contextual", "Censo INEP", dec=0),
    ...
    _i("equipes_esb", "Equipes de saúde bucal",
       "Equipes de saúde bucal no município", "Cobertura — saúde",
       "Equipes de saúde bucal cadastradas e ativas no CNES no município.",
       "equipes", "maior", "CNES/DATASUS", dec=0),
...
CAMPOS_NAO_INDICADORES = {
    ...
    "servicos_bucais", "dentistas_total",
}
