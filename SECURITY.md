...
| Discordância do recorte das equipes de saúde bucal | Abra uma issue. Contar junto as equipes de saúde bucal e as equipes de atenção básica que incluem saúde bucal é decisão editorial deste projeto — os rótulos de cada tipo casado ficam na proveniência justamente para permitir refazer a conta |
...
- **Âncoras de regressão.** Os totais conhecidos do Censo, do CPC e do CNES são
  testados a cada execução; mudança inesperada reprova o build.
- **Zero e ausência são testados nas duas direções.** Um teste garante que
  ausência nunca vire `0`; outro garante que o zero medido — as 27 UFs sem
  nenhuma vaga a distância — nunca vire "sem dados". Neste curso, apagar
  qualquer uma das duas apagaria o achado central.
- **A ausência de EaD é conferida na FONTE, não no resultado.** O conjunto
  publicado não distingue "o Censo não tem linha de EaD" de "o pipeline somou a
  camada errada"; por isso um teste abre o recorte bruto e confere que só
  existe a camada presencial. É a única checagem capaz de separar as duas
  coisas, e ela é o que substitui a guarda `vagas_ead > 0` que os observatórios
  irmãos usam.
...
- **Domínio incompleto não passa por domínio pronto.** As tabelas de nomes do
  CNES falham com frequência no FTP do DATASUS. Quando isso acontece, a falha
  fica registrada no cache e a execução seguinte tenta de novo **só o que
  faltou** — e, enquanto os tipos de equipe não forem lidos, o ICSB sai nulo em
  vez de zerado.
- **Guarda de riqueza.** O conjunto novo é comparado com o publicado; perda de
  campos aborta a publicação.
- **Acessibilidade medida.** O contraste de cada par que existe no CSS, a
  monotonicidade da escala sequencial e a coerência entre a paleta do CSS e a
  lista de reserva do JavaScript são testadas. Trocar a paleta é a forma mais
  silenciosa de tornar o site ilegível sem quebrar nada.
...
O projeto usa `jinja2`, `openpyxl`, `pandas`, `pillow` e `requests`, e traz
versionadas as bibliotecas de front-end (Leaflet, Chart.js) e as fontes. Nenhum
recurso é carregado de CDN em tempo de execução: o site funciona offline e não
expõe o leitor a terceiros.
