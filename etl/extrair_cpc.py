...
Odontologia: CPC, ENADE, IDD, perfil docente e as dimensões avaliadas pelos
estudantes.

Uso:
    python etl/extrair_cpc.py
    python etl/extrair_cpc.py --area ODONTOLOGIA --ciclo 2023

Saída: data/qualidade.json (por curso e agregado por UF).

ODONTOLOGIA ESTÁ NO CICLO DE 2023
----------------------------------
O CPC 2023 é o ciclo da saúde e das engenharias, e Odontologia está nele —
diferente de Psicologia e de Administração, avaliados em 2022. Medido nesta
planilha: 452 cursos em 27 UFs, 23.278 concluintes participantes, **todos
presenciais** (não há um único curso EaD avaliado). Procurar a área no ciclo
errado devolve "área não encontrada", e a leitura óbvia disso é "o curso não é
avaliado" — que seria falso.

A cobertura de indicador não é total, e a diferença importa: 448 cursos têm CPC
contínuo e faixa, 445 têm IDD, e 4 saem com faixa `SC` (sem conceito). Os
demais entram no site com o valor ausente declarado, nunca com zero.

O INVÓLUCRO DO ARQUIVO MUDA ENTRE CICLOS, E A FALHA SE DISFARÇA DE AUSÊNCIA
----------------------------------------------------------------------------
As 39 colunas são idênticas entre 2022 e 2023, mas o invólucro não:

  1. a caixa do nome do arquivo muda — 2023 publica `CPC_2023.xlsx`, 2022
     publica `cpc_2022.xlsx`. Pedir a grafia errada devolve 404, e a leitura
     óbvia do 404 é "este ciclo não existe";
  2. o nome da aba muda junto — `CPC_2023` com SUBLINHADO aqui, `CPC 2022` com
     ESPAÇO no ciclo anterior. Montar o nome com f"CPC_{ciclo}" quebra num dos
     dois casos;
  3. as colunas vêm com espaço à esquerda (' Ano', ' Área de Avaliação').

Por isso o nome do arquivo, o da aba e o das colunas são todos DESCOBERTOS, não
montados: `baixar_planilha`, `_aba_do_ciclo` e `_coluna` procuram em vez de
supor. Um extrator que supõe o invólucro quebra na próxima edição que mudar de
gosto — e quebra parecendo que a fonte sumiu.

AS ESCALAS SÃO TRÊS, E NENHUMA É A QUE O NOME SUGERE
------------------------------------------------------
Medido nesta planilha, para os 452 cursos de Odontologia:

  · dimensões avaliadas pelos estudantes (didático-pedagógica, infraestrutura,
    oportunidade de ampliação) — "Nota Bruta" na escala **1 a 6**; medido de
    2,838 a 6,000. O comentário herdado dos projetos irmãos dizia 0 a 5, e
    estava errado;
  · perfil docente (mestres, doutores, regime de trabalho) — "Nota Bruta" é
    **proporção de 0 a 1**; é ela que vira porcentagem com fator 100;
  · IDD — o publicado é a **Nota Padronizada**, de 0 a 5. A "Nota Bruta - IDD"
    vai de −9,316 a 13,165 nesta área e não serve para exibição nenhuma.
...
def vagas_por_curso(ano_censo):
    ...
    caminho = DADOS / f"curso_odontologia_{ano_censo}.csv"
...
def agregar(df, vagas, ciclo, url, area):
    ...
    for uf, grupo in df.groupby("uf"):
        ...
        ufs[str(uf).strip().upper()] = {
            ...
            # dimensões avaliadas pelos estudantes: notas brutas 1 a 6
            # (medido 2,838 a 6,000 nesta área), não 0 a 5.
...
def main():
    ...
    p.add_argument("--area", default="ODONTOLOGIA")
