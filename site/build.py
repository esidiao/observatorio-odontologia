"""
site/build.py
Gerador do site estático do Observatório Nacional da Formação em Odontologia.
...
# e aqui não servem por um motivo mais forte que nos irmãos: as 27 UFs valem
# exatamente zero. Uma variável constante não tem variância, e correlação sobre
# variância zero não é um número fraco — é indefinida. Calculá-la assim mesmo
# produziria `nan` ou, pior, um rho vindo de empates desfeitos por ordem de
# leitura. A ausência completa de EaD é o achado, e está dito na seção própria;
# o que ela não é, é variável explicativa.
FORA_DA_MATRIZ = {
    "pct_ead": "as 27 UFs valem zero; sem variância não há correlação a medir",
...
COMPONENTES_INDICE = [
    ...
    {"key": "ICAB", "peso": 20, "rotulo": "Cobertura — força de trabalho"},
    {"key": "ICRE", "peso": 15, "rotulo": "Cobertura — rede especializada"},
    {"key": "ICSB", "peso": 10, "rotulo": "Cobertura — saúde bucal na APS"},
...
def _soma_ou_nulo(ufs, campo):
    ...
    "0 municípios com dentista no SUS" — uma afirmação forte sustentada por um
...
def regressoes(ufs):
    ...
    if m:
        m["pergunta"] = ("A presença de cirurgiões-dentistas no SUS acompanha "
                         "a capacidade formativa do estado?")
        m["resposta"] = "ICAB"
        modelos.append(m)

    m = estatistica.regressao(
        serie("ICRE"),
        [serie("ICAB"), serie("ICSB"), serie("populacao")],
        ["Cobertura — profissional", "Cobertura — equipe na APS", "População"])
    if m:
        m["pergunta"] = ("As três coberturas andam juntas? Onde há dentista no "
                         "SUS e equipe de saúde bucal na atenção primária, há "
                         "também serviço especializado — ou a rede "
                         "especializada segue outra lógica, regionalizada?")
        m["resposta"] = "ICRE"
        modelos.append(m)
    return modelos
...
def construir(caminho_dados, saida, templates):
    ...
    panorama = {
        ...
        # Ofertas presenciais do país. O texto da home e o da página de estado
        # se apoiam neste número para dizer quantas ofertas foram lidas uma a
        # uma antes de afirmar que nenhuma é a distância.
        "n_cursos_presencial": soma("n_cursos_presencial"),
        ...
        # afirmaria "nenhum município do país tem dentista no SUS".
        "municipios_com_dentista": _soma_ou_nulo(ufs, "municipios_com_dentista"),
        "municipios_com_esp_bucal": _soma_ou_nulo(ufs, "municipios_com_esp_bucal"),
        "municipios_com_esb": _soma_ou_nulo(ufs, "municipios_com_esb"),
        "municipios_com_lrpd": _soma_ou_nulo(ufs, "municipios_com_lrpd"),
        "dentistas_sus": _soma_ou_nulo(ufs, "dentistas_sus"),
        "equipes_esb_total": _soma_ou_nulo(ufs, "equipes_esb_total"),
    ...
    comum = {
        ...
        # só aparecem em páginas municipais — `polos_ead`, `equipes_esb` — e
        ...
        # igual à do agregado estadual (no projeto irmão, "CRAS" aparecia
        # duas vezes por isso).
...
def escrever_pwa(saida, panorama, versao_estatica):
    manifesto = {
        ...
        "name": "Observatório Nacional da Formação em Odontologia",
        "short_name": "Obs. Odontologia",
        "lang": "pt-BR",
        "dir": "ltr",
        ...
        "description": "Indicadores de acesso territorial, qualidade e "
                       "cobertura assistencial dos cursos de Odontologia no "
